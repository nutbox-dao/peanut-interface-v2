...
import { setupNetwork, isUnlocked, connectMetamask } from "./web3";
...
export const getAbi = async function(contractName) {
  /* if (store.state.abis[contractName]) {
    return store.state.abis[contractName]
  } */
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (!onlyRead) {
      ...
      const contract = new ethers.Contract(
        contractAddress[contractName] || address,
        abi.abi,
        provider
      );
